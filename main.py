...
async def get_user(database, user_name: str):
    user_dict = await database.find_one({f"{user_name}.username": user_name})
    if user_dict:
...
@app.post("/")
async def root(current_user: Annotated[User, Depends(get_current_active_user)], long_url: Annotated[str, Form()], short_url: Optional[str] = Form(None)):
    url = await create_short_url(long_url, short_url, user_id=current_user.username)
    return url
...
@app.post("/{short_url}")
async def update_short_url(current_user: Annotated[User, Depends(get_current_active_user)], short_url: str, new_long_url: Annotated[str, Form()]):
    original_link = await db.links.find_one({'short_url': short_url})
    new_long_url = {'long_url': new_long_url}
    answer = await db.links.update_one({'_id': original_link['_id'], 'user_id': current_user.username}, {'$set': new_long_url})
